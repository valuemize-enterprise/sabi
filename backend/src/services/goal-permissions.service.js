...
async function editGoalDirect({ goalId, updates, caller }) {
  if (!SUPER_ROLES.has(caller.role)) {
    throw Object.assign(new Error('Only Super Admins can edit goals directly.'), { status: 403 });
  }

  const { data: before } = await supabase
    .from('brand_goals').select('*').eq('id', goalId).single();

  const allowed = ['objective', 'title', 'key_results', 'quarter', 'due_date', 'status'];
  ...
  const { data, error } = await supabase
    .from('brand_goals').update(patch).eq('id', goalId).select('*').single();

  if (error) throw new Error(`Failed to update goal: ${error.message}`);

  // Keep title in sync with objective for backward compat
  if (patch.objective && !patch.title) {
    await supabase.from('brand_goals').update({ title: patch.objective }).eq('id', goalId);
  }

  await audit({
    goalId, brandId: data.brand_id, actorId: caller.id,
    action: 'edited', summary: 'Super Admin edited goal directly',
...
async function deleteGoalDirect({ goalId, caller }) {
  if (!SUPER_ROLES.has(caller.role)) {
    throw Object.assign(new Error('Only Super Admins can delete goals directly.'), { status: 403 });
  }
  ...
  await audit({
    goalId, brandId: goal.brand_id, actorId: caller.id,
    action: 'deleted', summary: 'Super Admin deleted goal',
    before: goal,
  });

  // Soft-delete: mark as paused rather than hard-delete (preserves audit trail)
  const { error } = await supabase
    .from('brand_goals').update({ status: 'paused', last_edited_by: caller.id }).eq('id', goalId);
...
async function submitChangeRequest({ goalId, requestType, reason, proposedChanges, caller }) {
  ...
  if (existing) {
    throw Object.assign(
      new Error('You already have a pending request for this goal. Wait for a decision before submitting another.'),
      { status: 409 }
    );
  }

  const { data: goal } = await supabase
    .from('brand_goals').select('brand_id').eq('id', goalId).single();
  if (!goal) throw Object.assign(new Error('Goal not found.'), { status: 404 });

  // Get brand name for notification
  const { data: brand } = await supabase
    .from('brands').select('name').eq('id', goal.brand_id).single();
  ...
  if (superAdmins && superAdmins.length > 0) {
    const notifications = superAdmins.map(admin => ({
      ...
      message: `${caller.full_name} requested to ${requestType} a goal in ${brandName}`,
      link: `/brands/${goal.brand_id}?tab=goals`,
      metadata: {
        request_id: data.id,
        goal_id: goalId,
        brand_id: goal.brand_id,
    ...
    await supabase.from('notifications').insert(notifications).catch(err => {
      console.error('[goal-permissions] Failed to send notifications:', err.message);
    });
  }
...
async function decideChangeRequest({ requestId, approve, denialReason, caller }) {
  if (!SUPER_ROLES.has(caller.role)) {
    throw Object.assign(new Error('Only Super Admins can approve or deny change requests.'), { status: 403 });
  }
  ...
  if (crErr || !cr) throw Object.assign(new Error('Change request not found.'), { status: 404 });
  if (cr.status !== 'pending') {
    throw Object.assign(new Error(`This request has already been ${cr.status}.`), { status: 409 });
  }

  const decision = approve ? 'approved' : 'denied';

  await supabase.from('goal_change_requests').update({
    status:        decision,
  ...
  if (approve) {
    if (cr.request_type === 'delete') {
      ...
      if (cr.proposed_key_results) updates.key_results = cr.proposed_key_results;
    ...
    await audit({
      goalId: cr.goal_id, brandId: cr.brand_id, actorId: caller.id,
  ...
  await supabase.from('notifications').insert({
    ...
    console.error('[goal-permissions] Failed to send decision notification:', err.message);
  });
...
async function getPendingRequests(brandId, caller) {
  if (!SUPER_ROLES.has(caller.role)) {
    throw Object.assign(new Error('Only Super Admins can view all change requests.'), { status: 403 });
  }
