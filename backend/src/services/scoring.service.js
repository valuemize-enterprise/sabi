...
async function computeStaffScore(userId, weekStartDate, config) {
  ...
  const joinDate       = new Date(user.created_at); // platform onboarding date
...
if (weeksSinceJoin < 2) {
  ...
  let taskRate = null; // null = no tasks assigned this week
  ...
  const contribPoints  = Math.min(contributionPoints, wContrib > 15 ? wContrib : 15);
  const creativeBonus  = isCreativeOfWeek ? 5 : 0;

  const total = Math.round(
    (satisfactionPoints + taskPoints + managerPoints + contribPoints + creativeBonus) * 100
...
async function computeBrandAdminScore(userId, brandId, weekStartDate, config) {
  ...
  // FIX 3: 0 points for no tasks
  let taskRate = null;
  if (assignedTasks && assignedTasks.length > 0) {
    taskRate = assignedTasks.filter(t => t.status === 'verified').length / assignedTasks.length;
  }
  ...
  const wTask   = config.brand_admin_task_weight   + (satisfactionRaw === null ? Math.round(config.brand_admin_clarity_weight * 0.4) : 0);
  const wMgr    = config.brand_admin_rating_weight + (satisfactionRaw === null ? Math.round(config.brand_admin_clarity_weight * 0.6) : 0);
...
async function computeMissingScores(weeksBack = 8) {
  ...
  for (const week of weekRange) {
   ...
   // console.log(`[score] Processing week=${weekStr}`);
    ...
    for (const s of (staff ?? [])) {
      ...
      if (joinDate >= week) {
        // console.log(
        //   `[score] ${s.id} week=${weekStr} skipped=not_joined_yet`
        // );
        continue;
      }
      ...
      const result = await computeStaffScore(
        s.id,
        week,
        config
      );
      ...
      if (!result.excluded) {
        const rolling = await getRollingAverage(
          s.id,
          'staff'
        );
