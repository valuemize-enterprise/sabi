...
const p = (t, muted) =>
  `<p style="margin:0 0 14px;color:${muted ? C.soft : C.ink};font-size:${muted ? "13.5px" : "15px"};">${t}</p>`;
...
const ribbon = (level, text) => {
  const cfg = {
    ...
    2: {
      bg: "#fde8e8",
      bd: "#dc2626",
      label: "🔥 Second reminder — this is now urgent",
    },
    3: {
      bg: "#7f1d1d",
      bd: "#7f1d1d",
      label: "🚨 Final notice — leadership has been copied",
    },
...
T.task_assigned = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `<strong>${esc(d.assignedBy || "Your Brand Admin")}</strong> just assigned you a task on <strong>${esc(d.brandName)}</strong>:`,
      ) +
      infoBox("Task", esc(d.taskTitle)) +
      dueLine(d) +
      (d.strategyTitle
        ? p(
            `It belongs to the active strategy <strong>&ldquo;${esc(d.strategyTitle)}&rdquo;</strong> — read the strategy first so your work serves the plan.`,
            true,
          )
        : "") +
...
T.task_due_tomorrow = (d) => ({
  ...
  html: base(
    C.purple,
    "⏳",
    "Due tomorrow. Yes, tomorrow.",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `Friendly heads-up: <strong>&ldquo;${esc(d.taskTitle)}&rdquo;</strong> is due <strong>tomorrow</strong>. Deadlines have a funny way of arriving exactly on time — they have never once been late. 😅`,
      ) +
      dueLine(d) +
  ...
  return {
    ...
    html: base(
      ...
      ribbon(lvl, copy.ribbonText) +
        ...
        cta(
          lvl >= 3 ? "Resolve it right now →" : "Open the task →",
          `${APP}/tasks`,
          lvl >= 3 ? C.red : C.purple,
        ),
      `Overdue task on ${d.brandName}`,
      lvl >= 3
        ? `This was a final notice. Your Brand Admin has been copied so they can help you unblock it.`
        : undefined,
    ),
  };
};
...
T.task_rejected = (d) => ({
  category: "tasks",
  tone: "emotional+constructive",
  subject: `↩️ "${d.taskTitle}" came back — here's exactly why`,
  preheader: `Not a failure. A revision. The reason is written below.`,
  html: base(
    C.purple,
    "🛠️",
    "Sent back for one more pass",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `<strong>${esc(d.reviewerName || "Your reviewer")}</strong> reviewed <strong>&ldquo;${esc(d.taskTitle)}&rdquo;</strong> and sent it back to <strong>In Progress</strong>. Take a breath — this is the system working, not a verdict on you. Every rejection in Sabi must come with a written reason, and here it is:`,
      ) +
      infoBox(
        "Reason given",
        esc(d.reason || "See the task card for details"),
      ) +
...
T.task_verified = (d) => ({
  ...
  html: base(
    C.green,
    "🎉",
    "Your work passed the gate",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `<strong>&ldquo;${esc(d.taskTitle)}&rdquo;</strong> was just <strong>verified</strong> by ${esc(d.verifierName || "your Brand Admin")}. In Sabi, nothing self-reported counts — which means this ✅ is the real thing: your work was checked and it stood.`,
      ) +
      impactStrip(
...
T.claim_resolved = (d) => {
  ...
  return {
    category: "celebrations",
    tone: ok ? "celebration" : "constructive",
    subject: ok
      ? `⭐ Claim approved: +${d.points} points for going beyond`
      : `↩️ Your contribution claim wasn't approved this time`,
    preheader: ok
      ? `Extra effort, officially counted.`
      : `Here's the reasoning — and how to claim better next time.`,
    html: base(
      ok ? C.gold : C.purple,
      ok ? "⭐" : "📝",
      ok ? "Above and beyond — approved" : "About your contribution claim",
      esc(d.brandName || ""),
      greeting(d.name) +
        (ok
          ? p(
              `Your claim <strong>&ldquo;${esc(d.claimTitle)}&rdquo;</strong> was reviewed against your core functions and approved at <strong>${d.points} points</strong>. That means a real human looked at your proof and agreed: this was beyond your job. That&rsquo;s the culture we&rsquo;re building.`,
            )
          : p(
              `Your claim <strong>&ldquo;${esc(d.claimTitle)}&rdquo;</strong> was reviewed and not approved. The most common reason: the work falls <em>inside</em> your core functions — which means it earns task points instead, not contribution points.`,
            ) +
            infoBox(
              "Reviewer note",
              esc(d.reason || "Check the claim for details"),
            )) +
        (ok
          ? ""
          : steps("Claim smarter next time", [
        ...
        impactStrip(
          ...
          ok
            ? `Nothing — banked and counted.`
            : `Nothing lost — a declined claim never subtracts points.`,
          ok
            ? `+${d.points} points and a visible reputation as the person who shows up beyond the job description.`
            : `Sharper claims that get approved on the first try.`,
        ) +
        cta(
          ok ? "View my score →" : "Review my core functions →",
          ok ? `${APP}/my-score` : `${APP}/my-profile`,
          ok ? C.gold : C.purple,
        ),
...
T.weekly_score_published = (d) => ({
  ...
  html: base(
    ...
    "Weekly score published",
    greeting(d.name) +
      p(
        `Last week&rsquo;s score has been computed: <strong>${d.total}/100</strong>, and your 4-week rolling average — the number that actually matters — now stands at <strong>${d.rollingAvg}</strong>.`,
      ) +
      p(
        `Remember how this is built: client satisfaction, verified tasks, your manager&rsquo;s rating, and contributions. Every point traces to something real. One bad week never defines you; four good ones do.`,
        true,
      ) +
...
T.creative_of_week = (d) => ({
  ...
  html: base(
    ...
    "Chosen by the Creative Director",
    greeting(d.name) +
      p(
        `Out of every design, video and piece of creative logged this week, the Creative Director chose <strong>yours</strong>. A <strong>+5 bonus</strong> has been added to your score and your badge is glowing on the leaderboard.`,
      ) +
      (d.workTitle ? infoBox("The winning work", esc(d.workTitle)) : "") +
      p(
        `Take the moment. Screenshot the badge. Then come back Monday and defend the crown. 👑`,
        true,
      ) +
      impactStrip(
...
T.verification_queue = (d) => ({
  ...
  html: base(
    ...
    `${esc(d.brandName || "Your brands")} · ${d.count} pending`,
    greeting(d.name) +
      p(
        `<strong>${d.count} task(s)</strong> are sitting in <strong>Done</strong>, waiting for your verification. The oldest has waited <strong>${d.oldestDays} day(s)</strong>. Your team members earn <em>nothing</em> for this work until you click Verify — you are literally holding their points.`,
      ) +
...
T.verification_deadline = (d) => ({
  ...
  html: base(
    C.indigo,
    "⚠️",
    "Verify it today — tomorrow it costs you",
    esc(d.brandName),
    ribbon(2, `This task crosses the 5-day line tomorrow.`) +
      greeting(d.name) +
      p(
        `<strong>&ldquo;${esc(d.taskTitle)}&rdquo;</strong> (by ${esc(d.staffName)}) has been sitting in Done for <strong>${d.daysWaiting} days</strong>. From day 5, Sabi stops counting it as neutral — it counts <strong>against your own weekly score</strong>, by design, so verification never becomes the bottleneck.`,
      ) +
...
T.rate_team_reminder = (d) => ({
  ...
  html: base(
    C.indigo,
    "⭐",
    "Your team needs your rating",
    "Weekly ratings close Sunday night",
    greeting(d.name) +
      p(
        `You haven&rsquo;t rated <strong>${d.unratedCount} team member(s)</strong> this week. If Sunday midnight passes, Sabi records a neutral <strong>3/5</strong> for each of them — fair, but blind. A default can&rsquo;t see the designer who saved the campaign, or the writer who coasted.`,
      ) +
...
T.claims_pending = (d) => ({
  ...
  html: base(
    C.indigo,
    "📝",
    "Contribution claims awaiting you",
    `${d.count} pending`,
    greeting(d.name) +
      p(
        `<strong>${d.count} contribution claim(s)</strong> are waiting for review. Sabi shows you each claimant&rsquo;s core functions <em>side-by-side</em> with their claim — the &ldquo;is this actually extra?&rdquo; question takes ten seconds, not a debate.`,
      ) +
...
T.brief_received = (d) => ({
  ...
  html: base(
    C.amber,
    "📨",
    "A client just briefed us",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `<strong>${esc(d.brandName)}</strong> submitted a new brief: <strong>&ldquo;${esc(d.briefTitle)}&rdquo;</strong>. ARIA has already extracted the essentials:`,
      ) +
      infoBox(
        "ARIA\u2019s key insights",
        esc(d.ariaInsights || "Open the brief to view ARIA\u2019s analysis"),
      ) +
...
T.brief_unclassified = (d) => ({
  ...
  html: base(
    ...
    ribbon(
      d.level || 1,
      `Submitted ${d.hoursOld} hours ago. Still unclassified.`,
    ) +
      greeting(d.name) +
      p(
        `<strong>&ldquo;${esc(d.briefTitle)}&rdquo;</strong> from ${esc(d.brandName)} has sat unclassified for <strong>${d.hoursOld} hours</strong>. Until someone answers &ldquo;BAU or New Project?&rdquo;, this work has no price, no strategy, no owner — and the client is watching the silence.`,
      ) +
      impactStrip(
...
T.strategy_pending_approval = (d) => ({
  ...
  html: base(
    C.amber,
    "🧠",
    "A strategy needs your approval",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `The strategy <strong>&ldquo;${esc(d.strategyTitle)}&rdquo;</strong> for ${esc(d.brandName)} is drafted and waiting for your approval. Until you approve it, no tasks flow, no work starts, no points move — the entire execution engine idles on your decision.`,
      ) +
...
T.strategy_pnl_missing = (d) => ({
  ...
  html: base(
    C.amber,
    "💸",
    "This strategy has no price tag",
    esc(d.brandName),
    ribbon(
      d.level || 1,
      `The "P&L pending" badge is showing on this strategy.`,
    ) +
      greeting(d.name) +
      p(
        `<strong>&ldquo;${esc(d.strategyTitle)}&rdquo;</strong> is live-ish — but its P&amp;L is empty. No expected revenue. No estimated cost. Which means <strong>no invoice exists</strong> for this work, and the MD dashboard is reporting income that isn&rsquo;t being tracked.`,
      ) +
...
T.leadership_digest = (d) => ({
  ...
  html: base(
    ...
    esc(d.weekLabel || "Last week at Cerebre"),
    greeting(d.name) +
      p(`Here is last week, with nothing hidden:`) +
      infoBox(
        "Verified tasks across all brands",
        esc(String(d.verifiedTasks ?? "—")),
      ) +
      infoBox(
        "Expected revenue from new strategies",
        esc(d.newRevenue ?? "—"),
      ) +
      infoBox("Client satisfaction average", esc(d.satisfactionAvg ?? "—")) +
      (d.redFlags ? infoBox("🚩 Needs your attention", esc(d.redFlags)) : "") +
      p(
        `Every number above is built from verified work and real client responses — this is the same data the CEO sees.`,
        true,
      ) +
      impactStrip(
...
T.satisfaction_alert = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `<strong>${esc(d.brandName)}</strong> answered this week&rsquo;s satisfaction prompt with a <strong>${d.rating}/5</strong>. In Sabi, client satisfaction is the anchor metric — the client controls it, and it just moved against us.`,
      ) +
...
T.client_silent = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `<strong>${esc(d.brandName)}</strong> has skipped the satisfaction prompt for <strong>${d.weeksSilent} consecutive weeks</strong>. Sabi carries their last rating forward for up to three weeks — after that, the weight redistributes and we lose our clearest signal of how they feel.`,
      ) +
      p(
        `Silent clients are rarely neutral. They are either too busy to care (fine) or too disappointed to bother (dangerous). One call tells you which.`,
        true,
      ) +
...
T.creative_review_reminder = (d) => ({
  ...
  html: base(
    C.amber,
    "🎨",
    "Your creative review is ready",
    "The week in design & video",
    greeting(d.name) +
      p(
        `<strong>${d.itemCount} pieces</strong> of creative work from the past 7 days are queued on your review page — grouped by person, every proof link one click away. Somewhere in that list is this week&rsquo;s <strong>Creative of the Week</strong>.`,
      ) +
...
T.platform_digest = (d) => ({
  ...
  html: base(
    C.red,
    "🛡️",
    "Platform weekly report",
    esc(d.weekLabel || "The platform this week"),
    greeting(d.name) +
      infoBox("Active users this week", esc(String(d.activeUsers ?? "—"))) +
      infoBox(
        "Tasks verified / rejected",
        esc(`${d.tasksVerified ?? "—"} / ${d.tasksRejected ?? "—"}`),
      ) +
      infoBox("Expected revenue booked", esc(d.revenueBooked ?? "—")) +
      infoBox(
        "Briefs received / classified",
        esc(`${d.briefsIn ?? "—"} / ${d.briefsClassified ?? "—"}`),
      ) +
      (d.auditHighlights
        ? infoBox("🔎 Audit log highlights", esc(d.auditHighlights))
        : "") +
      impactStrip(
...
T.sensitive_action = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      ...
      p(
        `If you expected this, no action is needed. If you did not, review the audit log and the account involved immediately.`,
        true,
      ) +
      impactStrip(
...
T.client_report_published = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `A new intelligence report for <strong>${esc(d.brandName)}</strong> has been published to your portal. It was written by ARIA from your brand&rsquo;s verified work and live platform data — no fluff, no vanity metrics, just what moved and why.`,
      ) +
...
T.client_satisfaction_prompt = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `Every week, one question: <strong>how was our service?</strong> Here&rsquo;s the honest truth about your answer — it is the <em>anchor metric</em> of our internal scoring. Your one tap outweighs everything our team says about itself. That&rsquo;s by design: nothing self-reported counts at Cerebre.`,
      ) +
...
T.client_brief_confirmed = (d) => ({
  ...
  html: base(
    C.green,
    "✅",
    "Brief received — we&rsquo;re on it",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `Your brief <strong>&ldquo;${esc(d.briefTitle)}&rdquo;</strong> is in. Here&rsquo;s what happened the second you clicked submit: our leadership team was notified, and ARIA — our AI engine — read your brief and extracted the key insights so nothing you wrote gets missed.`,
      ) +
...
T.client_brief_status = (d) => ({
  ...
  html: base(
    C.green,
    "🔄",
    "Your brief moved forward",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `Status update on <strong>&ldquo;${esc(d.briefTitle)}&rdquo;</strong>:`,
      ) +
...
T.client_goal_achieved = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `It&rsquo;s official: <strong>&ldquo;${esc(d.goalTitle)}&rdquo;</strong> has been <strong>achieved</strong>. This wasn&rsquo;t luck — the Proof of Value trail in your portal shows exactly which work moved which metrics to get here.`,
      ) +
      impactStrip(
...
T.client_goal_at_risk = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `We promised you honesty over comfort, so here it is early: <strong>&ldquo;${esc(d.goalTitle)}&rdquo;</strong> has moved to <strong>at risk</strong>. VelocityTracker shows the current pace won&rsquo;t reach the target in time — and we&rsquo;d rather tell you now, when it&rsquo;s fixable, than explain it later.`,
      ) +
...
T.client_moment_upcoming = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `MomentMap has spotted something on the horizon: <strong>${esc(d.momentName)}</strong>${d.momentDate ? ` on <strong>${esc(d.momentDate)}</strong>` : ""}. Cultural moments are like Lagos traffic — you either plan for them or you sit in them watching others move. 😄`,
      ) +
...
T.client_invoice_reminder = (d) => ({
  ...
  html: base(
    C.green,
    "🧾",
    d.daysUntilDue <= 0 ? "An invoice is due" : "An invoice is coming due",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `A quick note from our accounts team: invoice <strong>${esc(d.invoiceRef)}</strong>${d.amount ? ` for <strong>${esc(d.amount)}</strong>` : ""} is ${d.daysUntilDue <= 0 ? "<strong>now due</strong>" : `due in <strong>${d.daysUntilDue} day(s)</strong>`}.`,
      ) +
...
T.brand_status_changed = (d) => ({
  ...
  html: base(
    C.red,
    "🛰️",
    "A brand crossed the line",
    esc(d.brandName),
    greeting(d.name) +
      p(
        `The Command Center rule engine just moved <strong>${esc(d.brandName)}</strong> from <strong>${esc(d.prevStatus === "watch" ? "Watch" : "Healthy")}</strong> to <strong style="color:${C.red};">At Risk</strong>. This is not a prediction — every reason below is a real record already breaching a standard the agency agreed on:`,
      ) +
      `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:6px 0 18px;border:1px solid ${C.line};border-radius:12px;">
...
T.profile_draft_ready = (d) => ({
  ...
  html: base(
    ...
    "Written for you, waiting on you",
    greeting(d.name) +
      p(
        `Welcome to Cerebre! While you were finding the kitchen, ARIA wrote the first draft of your team profile — bio, skills, the works — built from what HR told us about your role as <strong>${esc(d.roleTitle)}</strong>. Blank pages are for other agencies.`,
      ) +
...
T.profile_reminder = (d) => {
  ...
  const copy = {
    1: {
      subject: `👀 Your profile draft misses you (day ${d.daysWaiting})`,
    ...
    2: {
      subject: `⏰ Day ${d.daysWaiting}: clients see an empty seat where your face should be`,
    ...
    3: {
      subject: `🚨 Day ${d.daysWaiting}: your Brand Admin has been copied on this`,
  ...
  return {
    ...
    html: base(
      lvl >= 3 ? C.red : C.purple,
      "📇",
      "Your profile is still in draft",
      `Day ${d.daysWaiting}`,
      ...
      lvl >= 3
        ? `We will never publish on your behalf — but the checklist stays open until you do.`
        : undefined,
    ),
  };
};
...
T.role_updated = (d) => ({
  ...
  html: base(
    ...
    `${esc(d.prevTitle || "Previous role")} → ${esc(d.newTitle)}`,
    greeting(d.name) +
      p(
        `HR has updated your record: you are now <strong>${esc(d.newTitle)}</strong>. Sabi synced everything in one pass — your profile, your &ldquo;What&rsquo;s Expected of Me&rdquo; core functions, and the client Team pages for your brands. Nothing for you to edit.`,
      ) +
...
T.leave_request = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `<strong>${esc(d.staffName)}</strong> has requested <strong>${esc(d.leaveType)}</strong> leave from <strong>${esc(d.startDate)}</strong> to <strong>${esc(d.endDate)}</strong>.`,
      ) +
...
T.leave_decision = (d) => ({
  category: "people",
  tone: d.approved ? "celebration" : "informational",
  subject: d.approved
  ...
  preheader: d.approved
    ? `Approved by ${d.approverName}. Scoring exclusion is automatic.`
    : `Decision from ${d.approverName} inside.`,
  html: base(
    d.approved ? C.green : C.indigo,
    d.approved ? "🏖️" : "📋",
    d.approved ? "Your leave is approved" : "Your leave request was declined",
    esc(d.leaveType + " leave"),
    greeting(d.name) +
      (d.approved
        ? p(
            `<strong>${esc(d.approverName)}</strong> approved your ${esc(d.leaveType)} leave from <strong>${esc(d.startDate)}</strong> to <strong>${esc(d.endDate)}</strong>. Sabi has already excluded those weeks from your scoring — rest without the math anxiety.`,
          )
        : p(
            `<strong>${esc(d.approverName)}</strong> couldn&rsquo;t approve this one. The reason is below — talk to them directly if timing can flex.`,
          ) + infoBox("Reason", esc(d.decisionNote || "See your approver"))) +
      impactStrip(
        d.approved
          ? `Leave weeks are fully excluded from your 4-week rolling average — by design.`
          : `Declined requests cost you nothing on your score.`,
        d.approved
          ? `Just one thing: hand over open tasks before you go — verified beats &ldquo;almost done&rdquo;.`
          : `Nothing — resubmit with new dates anytime.`,
        d.approved
          ? `Actual rest. The system holds your place.`
          : `A clear reason and a fast path to a better slot.`,
      ) +
      cta(
        d.approved ? "View my leave →" : "Request new dates →",
        `${APP}/my-profile`,
        d.approved ? C.green : C.indigo,
      ),
...
T.probation_ending = (d) => ({
  ...
  html: base(
    C.amber,
    "⏳",
    "A probation decision is due",
    esc(d.personName),
    greeting(d.name) +
      p(
        `<strong>${esc(d.personName)}</strong> (${esc(d.roleTitle)}) completes probation on <strong>${esc(d.probationEnd)}</strong>. In seven days they either become a confirmed part of this team or they don&rsquo;t — and the worst outcome is the one that happens by nobody deciding.`,
      ) +
...
T.document_expiring = (d) => ({
  ...
  html: base(
    d.daysLeft <= 7 ? C.red : C.amber,
    "📁",
    "A document is expiring",
    esc(d.personName),
    ribbon(
      d.level || 1,
      d.daysLeft <= 0 ? "Already expired." : `${d.daysLeft} days remaining.`,
    ) +
      greeting(d.name) +
      p(
        `<strong>${esc(d.personName)}</strong>&rsquo;s <strong>${esc(d.docLabel)}</strong> (${esc(d.docType)}) ${d.daysLeft <= 0 ? "has <strong>expired</strong>" : `expires in <strong>${d.daysLeft} days</strong>`}.`,
      ) +
      impactStrip(
...
T.work_anniversary = (d) => ({
  ...
  html: base(
    C.gold,
    "🎉",
    `Happy ${d.years}-year anniversary!`,
    "Cerebre Media Africa",
    greeting(d.name) +
      p(
        `<strong>${d.years} year${d.years > 1 ? "s" : ""} ago today</strong>, you joined this team. Since then: verified work with your name on it, clients who know your face from the Team page, and a score history that tells the real story. Thank you for building this with us. 🥂`,
      ) +
      impactStrip(
...
T.birthday = (d) => ({
  ...
  html: base(
    C.gold,
    "🎂",
    `Happy birthday, ${esc(d.name)}!`,
    "From all of us at Cerebre",
    greeting(d.name) +
      p(
        `It&rsquo;s your day. The tasks will wait, the briefs will wait, even ARIA will wait. Have something sweet, take the long lunch, and accept the team&rsquo;s terrible singing with grace. 🎈`,
      ) +
      impactStrip(
...
T.offboarding_started = (d) => ({
  ...
  html: base(
    C.red,
    "📤",
    "An offboarding has begun",
    esc(d.personName),
    greeting(d.name) +
      p(
        `HR has started offboarding for <strong>${esc(d.personName)}</strong> (${esc(d.roleTitle)}). The kill switch ran the full checklist automatically:`,
      ) +
      infoBox(
        "Completed automatically",
        "Account deactivated · profile removed from client Team pages · brand assignments released · exit recorded",
      ) +
      infoBox(
        "Needs a human",
        `${d.openTasks} open task(s) unassigned — Brand Admins should reassign them this week`,
      ) +
      impactStrip(
...
T.profile_form_submitted = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name || "HR") +
      p(
        `<strong>${esc(d.staffName)}</strong> has completed and submitted their Staff Profile Form. The record is now waiting for HR review and verification.`,
      ) +
      infoBox("Staff email", esc(d.staffEmail)) +
      infoBox("Submitted at", esc(d.submittedAt)) +
      p(
        "Once you've verified the information is accurate, mark it as verified in People OS. If there are issues, you can reject it with a note explaining what needs to be corrected.",
        true,
      ) +
...
T.profile_form_submitted_ack = (d) => ({
  ...
  html: base(
    ...
    "✅",
    "Profile submitted",
    "Your record is with HR",
    greeting(d.name) +
      p(
        `Your Staff Profile Form has been successfully submitted. HR has been notified and will verify your details.`,
      ) +
      ...
      p(
        `Meanwhile, you can publish your bio profile so clients can see you on your brand\'s Team page.`,
        true,
      ) +
...
T.profile_form_verified = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      p(
        `HR has reviewed your Staff Profile Form and everything checks out. Your official record is now on file.`,
      ) +
      (d.hrNotes ? infoBox("Note from HR", esc(d.hrNotes)) : "") +
      p(
        "Your profile is now part of Cerebre's official records. If you ever need to update your contact details, next of kin, or any other personal information in the future, you can do so from your profile page.",
        true,
      ) +
...
T.profile_form_rejected = (d) => ({
  ...
  html: base(
    C.amber,
    "⚠️",
    "Profile needs a correction",
    "Action required",
    greeting(d.name) +
      p(
        `HR has reviewed your Staff Profile Form and found something that needs to be corrected before your record can be verified.`,
      ) +
      infoBox("Reason from HR", esc(d.reason)) +
      p(
        "Please go to My Profile, update the information flagged above, and re-submit your form. Your previous entries will still be there — you only need to fix what was flagged.",
        true,
      ) +
...
T.profile_form_nag = (d) => ({
  ...
  html: base(
    ...
    greeting(d.name) +
      (d.level === 1
        ? p(
            `You joined Cerebre ${d.daysSince} days ago — your official Staff Profile Form is ready and waiting for you. It covers your personal details, qualifications, work history, and medical information that HR keeps on file.`,
          )
        : d.level === 2
          ? p(
              `Your Staff Profile Form has been pending for <strong>${d.daysSince} days</strong>. Without it, HR cannot complete your official record. This is now affecting your onboarding status.`,
            )
          : p(
              `<strong>Your Staff Profile Form is ${d.daysSince} days overdue.</strong> This is now flagged at leadership level. HR needs your completed form to finalise your employment record.`,
            )) +
      p(
        "The form covers: personal details, family & next of kin, medical information (confidential), education, languages, work history, and your guarantor details.",
        true,
      ) +
      impactStrip(
        "An incomplete record means HR cannot contact the right person in an emergency.",
        d.level >= 2
          ? "Your probation review cannot proceed without a complete record on file."
          : "It blocks your onboarding from being marked complete.",
        "A verified record = a proper start at Cerebre.",
      ) +
      cta(
        "Complete my profile now →",
        `${APP}/my-profile`,
        d.level >= 3 ? C.red : C.volt,
      ),
    ...
    html: base(
      ...
      <tr><td style="padding:32px 40px 0;">
        <p style="margin:0 0 8px;font-family:'JetBrains Mono',Menlo,monospace;
                  ...
                  text-transform:uppercase;color:#64748b;">Action Required</p>
        <h1 style="margin:0 0 16px;font-family:'Space Grotesk',Arial,sans-serif;
                   font-size:22px;font-weight:700;color:#f1f5f9;line-height:1.25;">
          Goal Change Request<br>Needs Your Approval
        </h1>
        <p style="margin:0 0 24px;font-family:Inter,Arial,sans-serif;
                  font-size:14px;color:#94a3b8;line-height:1.7;">
          Hi <strong style="color:#e2e8f0;">${d.recipientName}</strong>,
          <strong style="color:#e2e8f0;">${d.requesterName}</strong>
          has requested to <strong style="color:#e2e8f0;">${d.requestType}</strong>
          a goal in <strong style="color:#e2e8f0;">${d.brandName}</strong>.
        </p>
      </td></tr>

      <tr><td style="padding:0 40px 24px;">
        <table width="100%" cellpadding="0" cellspacing="0" border="0"
               style="background:#1e1e35;border:1px solid #2d2d4a;border-radius:10px;">
          <tr><td style="padding:20px 24px;">

            <!-- action badge — uses local badge variable, not d.badge -->
            <table cellpadding="0" cellspacing="0" border="0" style="margin-bottom:18px;">
              <tr><td style="background:${badge.bg};border:1px solid ${badge.border};
                              border-radius:6px;padding:4px 12px;
                              font-family:'JetBrains Mono',Menlo,monospace;
                              font-size:11px;font-weight:700;color:${badge.text};
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr>
                <td width="35%" style="font-family:Inter,Arial,sans-serif;font-size:12px;
                                       color:#64748b;padding-bottom:10px;">Brand</td>
                <td style="font-family:Inter,Arial,sans-serif;font-size:12px;
                           font-weight:600;color:#e2e8f0;padding-bottom:10px;">
                  ${d.brandName}
                </td>
              </tr>
              <tr>
                <td width="35%" style="font-family:Inter,Arial,sans-serif;font-size:12px;
                                       color:#64748b;border-top:1px solid #2d2d4a;
                                       padding:10px 0;">Goal</td>
                <td style="font-family:Inter,Arial,sans-serif;font-size:12px;
                           font-weight:600;color:#e2e8f0;
                           border-top:1px solid #2d2d4a;padding:10px 0;">
                  ${d.goalTitle}
                </td>
              </tr>
              <tr>
                <td width="35%" style="font-family:Inter,Arial,sans-serif;font-size:12px;
                                       color:#64748b;border-top:1px solid #2d2d4a;
                                       padding-top:10px;">Request type</td>
                <td style="font-family:Inter,Arial,sans-serif;font-size:12px;
                           font-weight:600;color:#e2e8f0;
                           border-top:1px solid #2d2d4a;padding-top:10px;">
                  ${actionLabel}
                </td>
              </tr>
            </table>
...
T.guarantor_form_reminder = (d) => ({
  ...
  html: base(
    C.amber,
    "🤝",
    "Guarantor form outstanding",
    esc(d.staffName),
    greeting(d.name || "HR") +
      p(
        `The physical Guarantor's Form for <strong>${esc(d.staffName)}</strong> has not yet been received. Their probation review is in <strong>${d.daysUntilProbation} days</strong>.`,
      ) +
      infoBox("Guarantor", esc(d.guarantorName)) +
      infoBox("Guarantor phone", esc(d.guarantorPhone)) +
      infoBox(
        "Guarantor email",
        d.guarantorEmail ? esc(d.guarantorEmail) : "Not provided",
      ) +
      infoBox("Probation review date", esc(d.probationDate)) +
      p(
        "The form must be physically signed, returned to HR, and filed before the probation review can proceed. Chase the guarantor now to avoid delaying the review.",
        true,
      ) +
