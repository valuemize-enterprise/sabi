...
async function getRecord(subjectUserId, caller, context = "person_file") {
  ...
  else if (T3_AUDITED_ROLES.has(caller.role))
    fields = TIER3_FIELDS; // logged below
  else if (isSelf)
    fields = TIER3_FIELDS; // own record, own data
  else if (caller.role === "admin" || caller.role === "md")
    fields = TIER2_FIELDS;
  ...
  if (
    data &&
    fields === TIER3_FIELDS &&
    T3_AUDITED_ROLES.has(caller.role) &&
    !isSelf
  ) {
...
async function createRecord(input, creator) {
  ...
  if (!user) {
    ...
    try {
      ...
      if (emailService?.sendStaffInvite)
        await emailService.sendStaffInvite(user);
    } catch {
      /* invite service optional in dev */
    }
  }

  // 2 · the People Record
  const onboarding = Object.fromEntries(
    ONBOARDING_STEPS.map((s) => [s, false]),
  );
  ...
  generateProfile(user.id).catch((e) =>
    console.error("[people] generate failed:", e.message),
  );
...
async function updateRecord(subjectUserId, patch, caller) {
  ...
  const { data: before } = await supabase
    .from("people_records")
    .select("role_key, role_title, display_name")
    .eq("user_id", subjectUserId)
    .single();
  ...
  if (before && (clean.role_key || clean.role_title || clean.display_name)) {
    ...
    if (roleChanged) {
      const { data: u } = await supabase
        ...
        .single();
      if (u)
        await dispatch.send("role_updated", {
          to: u,
          entityId: subjectUserId,
          dedupe: "always",
          data: {
            name: u.full_name,
            newTitle: after.role_title,
            prevTitle: before.role_title,
          },
        });
    }
    await supabase
...
async function registry(caller) {
  const fields =
    HR_ROLES.has(caller.role) || T3_AUDITED_ROLES.has(caller.role)
      ? TIER2_FIELDS
      : TIER1_FIELDS;
  const [
    { data: records },
    ...
    { data: docs },
    { data: staff_profile, error: staff_error },
  ] = await Promise.all([
    supabase
      .from("people_records")
      .select(sel(fields))
  ...
  const onLeaveNow = new Set(
    (leaves || [])
      .filter(
        (l) => new Date(l.start_date) <= today && new Date(l.end_date) >= today,
      )
      .map((l) => l.user_id),
  );
  ...
  for (const d of docs || []) {
    ...
    if (exp <= soon)
      expiringByUser.set(d.user_id, (expiringByUser.get(d.user_id) || 0) + 1);
  }
  const people = (records || []).map((r) => {
    const prof = profByUser.get(r.user_id);
    const draftDays =
      prof?.state === "draft" && prof.generated_at
        ? Math.floor((today - new Date(prof.generated_at)) / 86400000)
        : null;
    return {
      ...r,
      ...
      probation_active: r.probation_end
        ? new Date(r.probation_end) >= today
        : false,
  ...
  return {
    ...
    stats: {
      ...
      drafts_unclaimed: people.filter((p) => p.profile_state === "draft")
        .length,
    },
  };
}
  ...
  if (hrEmailList.length) {
    await dispatch.send("profile_form_submitted", {
      ...
      data: {
        ...
        submittedAt: now.toLocaleString("en-NG", {
...
async function personFile(subjectUserId, caller) {
  ...
  const canT3 =
    HR_ROLES.has(caller.role) ||
    T3_AUDITED_ROLES.has(caller.role) ||
    caller.id === subjectUserId;

  const [profQ, leaveQ, docsQ, scoreQ, ratingsQ, recogQ, disputesQ] =
    await Promise.all([
      supabase
        .from("staff_profiles")
        .select("state, generated_at, published_at, generation_version")
        .eq("user_id", subjectUserId)
        .maybeSingle(),
      supabase
        .from("leave_requests")
        .select("id, leave_type, start_date, end_date, status, decision_note")
        .eq("user_id", subjectUserId)
      ...
      supabase
        ...
        .eq("ratee_id", subjectUserId)
        .lte("rating", 2)
        .order("created_at", { ascending: false })
        .limit(5),
      supabase
        .from("contribution_claims")
        .select("title, points, status, created_at")
        .eq("user_id", subjectUserId)
        .eq("status", "approved")
        .order("created_at", { ascending: false })
        .limit(5),
  ...
  return {
    ...
    performance: {
      // HR lens (D2): averages + flags, no weekly detail
...
async function insights(caller) {
  ...
  const { data: records } = await supabase
    .from("people_records")
    .select(
      "role_key, employment_type, start_date, status, date_of_birth, display_name, user_id",
    );
  ...
  return {
    ...
    by_role: byRole,
    by_type: byType,
    avg_tenure_years: active.length
      ? +(tenureSum / active.length).toFixed(1)
      : 0,
    exited_total: (records || []).length - active.length,
    upcoming_anniversaries: active
      .filter((r) =>
        in30(
          new Date(r.start_date).getMonth(),
    ...
    upcoming_birthdays: active // day+month only — year never surfaced (D6)
      .filter(
        (r) =>
          r.date_of_birth &&
          in30(
            new Date(r.date_of_birth).getMonth(),
            new Date(r.date_of_birth).getDate(),
          ),
      )
      .map((r) => ({
        name: r.display_name,
        day: new Date(r.date_of_birth).toLocaleDateString("en-NG", {
          day: "numeric",
          month: "long",
        }),
      })),
