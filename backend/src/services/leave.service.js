...
async function requestLeave(
  user,
  { leave_type = "annual", start_date, end_date, note = null },
) {
  ...
  const approvers = [];
  for (const u of [
    ...(bas || []).map((r) => r.users).filter(Boolean),
    ...(hr || []),
  ]) {
  ...
  await dispatch
    ...
    .catch((e) =>
      console.error("[leave] staff confirmation email failed:", e.message),
    );
...
async function decideLeave(requestId, approver, approve, decisionNote = null) {
  ...
  const allowed =
    DIRECT_APPROVERS.has(approver.role) ||
    (await isTeamBrandAdmin(approver.id, req.user_id));
  if (!allowed)
    throw Object.assign(new Error("Not authorised to approve this request"), {
      status: 403,
    });
  ...
  if (approve) {
    ...
    if (rows.length) {
      await supabase
        .from("staff_leave")
        .upsert(rows, { onConflict: "staff_id,week_start" });
    }
  }
  ...
  const { data: staff } = await supabase
    ...
    .single();
  if (staff) {
    await dispatch.send("leave_decision", {
      to: staff,
      entityId: requestId,
      dedupe: "once",
      data: {
...
await notify(
  ...
  approve ? 'Leave Approved' : 'Leave Request Rejected',
  approve
    ? `Your ${req.leave_type} leave from ${req.start_date} to ${req.end_date} has been approved.`
    : `Your leave request was not approved. ${decisionNote || ''}`.trim(),
  { leave_id: requestId }
);
  }
  return { status };
...
async function attachUserNames(requests) {
  ...
  return rows.map((r) => ({
    ...r,
    user: byId.get(r.user_id) || { full_name: null },
  }));
}
