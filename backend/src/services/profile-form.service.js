...
async function appendHistory(
  ...
  await supabase
    ...
    .then(
      () => {},
      (e) => console.error("[profile-history]", e?.message),
    );
}
...
async function getFormForHR(callerId, subjectUserId, callerRole) {
  const fields =
    HR_ROLES.has(callerRole) || callerRole === "super_admin"
      ? TIER3_FIELDS
      : callerRole === "md"
    ...
    .upsert(
      {
        ...
        user_id: userId,
        profile_state:
          clean.profile_state === "submitted" ? "submitted" : "draft",
  ...
  if (hrEmailList.length) {
    await dispatch.send("profile_form_submitted", {
      ...
      data: {
        ...
        submittedAt: new Date(now).toLocaleString("en-NG", {
...
async function rejectForm(callerId, subjectUserId, reason) {
  if (!reason?.trim()) {
    const e = new Error(
      "A rejection reason is required so the staff member knows what to fix.",
    );
    e.status = 400;
    throw e;
  }
  ...
  await dispatch.send("profile_form_rejected", {
    ...
    data: {
      ...
      reason,
      editUrl: `${process.env.APP_URL}/my-profile`,
    },
  });
