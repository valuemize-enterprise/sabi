...
router.get("/to-rate", authenticate, async (req, res, next) => {
  try {
    ...
    if (isGlobalAdmin(req.user.role)) {
      ...
      brandIds = (assignments ?? [])
        .filter((a) => (a.roles_on_brand ?? []).includes("brand_admin"))
        .map((a) => a.brand_id);
    }
    ...
    const { data: team } = await supabase
      .from("staff_brand_assignments")
      .select(
        "staff_id, brand_id, brands(name), users!staff_id(id, full_name, role)",
      )
      .in("brand_id", brandIds);

    const { data: alreadyRated } = await supabase
      .from("weekly_ratings")
      .select("staff_id, brand_id")
      .eq("rater_id", req.user.id)
      .eq("week_start", week);

    const ratedSet = new Set(
      (alreadyRated ?? []).map((r) => `${r.staff_id}_${r.brand_id}`),
    );
    const toRate = (team ?? []).filter(
      (t) =>
        !ratedSet.has(`${t.staff_id}_${t.brand_id}`) &&
        t.staff_id !== req.user.id,
    );
...
router.post("/", authenticate, async (req, res, next) => {
  try {
    ...
    if (!staff_id || !brand_id)
      return sendError(res, 400, "staff_id and brand_id are required");
    if (!score || score < 1 || score > 5)
      return sendError(res, 400, "score must be between 1 and 5");
    if (score <= 2 && !note?.trim())
      return sendError(
        res,
        400,
        "A note is required when rating 2 or below — this protects the staff member with context",
      );
    ...
    if (!allowed)
      return sendError(
        res,
        403,
        "Only the Brand Admin or a global admin can rate this brand's team",
      );
    ...
    const { data, error } = await supabase
      ...
      .upsert(
        {
          rater_id: req.user.role !== "super_admin" ? req.user.id : null,
          staff_id,
          brand_id,
          week_start: week,
          score,
          note: note || null,
          is_creative_of_week: !!is_creative_of_week,
        },
    ...
    if (is_creative_of_week)
      notify.onCreativeOfWeek(staff_id, note || "Creative work");
...
router.get("/mine", authenticate, async (req, res, next) => {
  try {
    const { data, error } = await supabase
      .from("weekly_ratings")
      .select(
        "score, note, week_start, is_creative_of_week, brands(name), rater:users!rater_id(full_name)",
      )
      .eq("staff_id", req.user.id)
...
router.get("/creative-review-data", authenticate, async (req, res, next) => {
  try {
    ...
    if (
      !CREATIVE_REVIEW_ROLES.includes(req.user.role) &&
      !isGlobalAdmin(req.user.role)
    ) {
      return sendError(
        res,
        403,
        "Only the Creative Director, MD, and global admins can access creative review",
      );
    }

    const CREATIVE_STAFF_ROLES = [
      "art_director",
      "content_creator",
      "creative_lead",
    ];
    ...
    const { data: creativeStaff } = await supabase
      .from("staff_brand_assignments")
      .select("id, staff_id, role_on_brand")
      .in("role_on_brand", CREATIVE_STAFF_ROLES);

    // console.log('creativeStaff', creativeStaff);
    const creativeIds = (creativeStaff ?? []).map((s) => s.staff_id);
    ...
    // console.log('creativeIds', creativeIds);

    const { data: workLogs, error } = await supabase
      .from("work_logs")
      .select(
        "id, title, description, category,  evidence_files, proof_links, created_at, user_id, brand_id, brands(name)",
      )
    ...
    const staffMap = Object.fromEntries(
      (creativeStaff ?? []).map((s) => [s.staff_id, s]),
    );
    ...
    (workLogs ?? []).forEach((log) => {
      ...
      if (!byStaff[sid])
        byStaff[sid] = {
          staff: staffMap[sid] ?? { id: sid, full_name: "Unknown" },
          entries: [],
        };
