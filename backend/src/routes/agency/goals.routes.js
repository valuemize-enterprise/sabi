...
router.post(
  "/",
  authenticate,
  requireBrandAdminOrPermission("CREATE_GOAL"),
  async (req, res, next) => {
    try {
      const {
        brand_id,
      ...
      if (!brand_id || !title || !metric_type || target_value === undefined) {
        return sendError(
          res,
          400,
          "brand_id, title, metric_type, and target_value required",
        );
      }
...
router.get("/:id", authenticate, async (req, res, next) => {
  try {
    const { data, error } = await supabase
      .from("goals")
      .select(
        "*, brands(name, logo_url), tasks(id, title, status, completed_at)",
      )
      .eq("id", req.params.id)
      .single();
...
router.put(
  "/:id",
  authenticate,
  requireBrandAdminOrPermission("EDIT_GOAL"),
  async (req, res, next) => {
    try {
      const allowed = [
        "title",
      ...
      console.log("Updating goal", req.params.id, "with:", updates);
      ...
      if (updates.status === "achieved" || updates.status === "at_risk") {
        notify.onGoalStatusChanged(
          { id: data.id, title: data.title, brand_id: data.brand_id },
          updates.status,
        );
      }
      sendSuccess(res, { goal: data });
...
router.post("/:id/track-velocity", authenticate, async (req, res, next) => {
  try {
    ...
    const result = await velocityService.analyze({
      goal,
      tasks: tasks || [],
      brand: goal.brands,
    });
    ...
    await supabase
      .from("goals")
      .update({ velocity_score: result.velocityScore, velocity_data: result })
      .eq("id", req.params.id);
