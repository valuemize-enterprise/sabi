...
router.get("/", authenticate, async (req, res, next) => {
  try {
    ...
    let weekFilter = {};
    if (period === "week") {
      const lastWeek = scoringService.toDateStr(
        scoringService.lastCompletedWeekStart(),
      );
      weekFilter = { week_start: lastWeek };
    } else if (period === "month") {
      const monthAgo = scoringService.toDateStr(
    ...
    if (period === "all") {
      ...
      entries = await Promise.all(
        userIds.map(async (uid) => {
          ...
          // console.log('user', user, 'avg', avg, 'prevAvg', prevAvg);
      ...
      let query = supabase
        .from("weekly_scores")
        .select(
          "user_id, total, excluded, week_start, users!user_id(id, full_name, avatar_url, role)",
        )
        .eq("score_type", type)
        .eq("excluded", false);
      if (weekFilter.week_start)
      ...
      let prevQuery = supabase
        .from("weekly_scores")
        .select("user_id, total, excluded, week_start")
        .eq("score_type", type)
        .eq("excluded", false);
      if (period === "week") {
        const prevWeekStart = scoringService.toDateStr(
          new Date(
            scoringService.lastCompletedWeekStart().getTime() - 7 * 86400000,
          ),
        );
        prevQuery = prevQuery.eq("week_start", prevWeekStart);
      } else {
        const prevMonthStart = scoringService.toDateStr(
          new Date(Date.now() - 56 * 86400000),
        );
        const prevMonthEnd = scoringService.toDateStr(
          new Date(Date.now() - 28 * 86400000),
        );
        prevQuery = prevQuery
      ...
      if (period === "month") {
        ...
        entries = Object.values(byUser).map((v) => {
          ...
          const prevScore = prevScores?.length
            ? prevScores.reduce((s, n) => s + n, 0) / prevScores.length
            : null;
        ...
        const prevByUser = {};
        (prevData ?? []).forEach((row) => {
          prevByUser[row.user_id] = Number(row.total);
        });
