...
router.put("/:id/verify", authenticate, async (req, res, next) => {
  try {
    const { data: task } = await supabase
      .from("tasks")
      .select("id, title, status, brand_id, assignee_id")
      .eq("id", req.params.id)
      .single();
    if (!task) return sendError(res, 404, "Task not found");
    if (task.status !== "done")
      return sendError(res, 400, 'Only tasks marked "Done" can be verified');
    ...
    if (!allowed)
      return sendError(
        res,
        403,
        "Only the Brand Admin or a global admin can verify tasks for this brand",
      );
    ...
    if (task.assignee_id) {
      require("../../services/people.service")
        .setOnboardingStep(task.assignee_id, "first_task_done")
        .catch((e) =>
  ...
  try {
    ...
    if (!reason?.trim())
      return sendError(
        res,
        400,
        "A reason is required when sending a task back",
      );
    ...
    if (!allowed)
      return sendError(
        res,
        403,
        "Only the Brand Admin or a global admin can review this task",
      );
...
router.get("/pending-verification", authenticate, async (req, res, next) => {
  try {
    ...
    if (brandIds.length === 0)
      return sendSuccess(res, { tasks: [], overdueCount: 0 });

    const { data: tasks, error } = await supabase
      .from("tasks")
      .select(
        "id, title, status, updated_at, brand_id, brands(name), assignee:users!assignee_id(id, full_name)",
      )
    ...
    const { data: config } = await supabase
      .from("scoring_config")
      .select("unverified_task_grace_days")
      .eq("id", 1)
      .single();
