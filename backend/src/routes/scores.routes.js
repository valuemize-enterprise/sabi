...
router.get("/mine", authenticate, async (req, res, next) => {
  ...
  try {
    ...
    const createdAt = new Date(user.created_at); // FIX: use created_at (join date), not updated_at
    const twoWeeksAgo = new Date();
    twoWeeksAgo.setDate(twoWeeksAgo.getDate() - 14);

    if (createdAt > twoWeeksAgo) {
      // FIX: block if joined AFTER the 14-day cutoff (i.e. too recent)
      return sendError(
        res,
        400,
        "Only users with 2+ weeks on the app can view scores",
      );
    }
    ...
    if (scoreType === "staff") {
      const d1 = await scoringService.scoreCompute(ids, weekStart, config);
      scoreData = d1;
    } else {
    ...
    const rollingAvg = await scoringService.getRollingAverage(
      ids,
      scoreType,
    );
    ...
    sendSuccess(res, {
      scoreType,
      rollingAverage:
        rollingAvg != null ? Math.round(rollingAvg * 100) / 100 : null,
...
router.post("/:userId", async (req, res) => {
  try {
    ...
    if (!score || !week_start) {
      return res
        .status(400)
        .json({ error: "score and week_start are required" });
    }
