...
router.get('/opportunities', requireRoles(...PIPELINE_READERS), async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to load pipeline', message: err.message });
  }
});

// POST /api/pipeline/opportunities
// Create a new opportunity
router.post('/opportunities', requireRoles(...PIPELINE_READERS), async (req, res) => {
  try {
    const {
      company_name, deal_title, description, service_types,
      source, stage, estimated_value, date_briefed,
      client_deadline, agency_deadline, lead_ba_id,
      accountable_team_text, notes,
    } = req.body;
    ...
    const opp = await pipelineService.createOpportunity(
      {
        ...
        description, service_types, source, stage,
        estimated_value, date_briefed, client_deadline, agency_deadline,
    ...
    res.status(500).json({ error: 'Failed to create opportunity', message: err.message });
  }
});

// GET /api/pipeline/opportunities/:id
// Get one opportunity with full detail (stage history + weekly notes)
router.get('/opportunities/:id', requireRoles(...PIPELINE_READERS), async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to load opportunity', message: err.message });
  }
});

// PATCH /api/pipeline/opportunities/:id
// Update fields (not stage — use /stage endpoint for that)
router.patch('/opportunities/:id', requireRoles(...PIPELINE_READERS), async (req, res) => {
  try {
    const existing = await pipelineService.getOpportunityById(req.params.id);
    if (!existing) return res.status(404).json({ error: 'Opportunity not found' });

    if (req.user.role === 'brand_admin' && existing.lead_ba_id !== req.user.id) {
      return res.status(403).json({ error: 'You can only edit your own opportunities' });
    }

    const updated = await pipelineService.updateOpportunity(req.params.id, req.body);
    res.json({ opportunity: updated });
  } catch (err) {
    console.error('[Pipeline] updateOpportunity error:', err);
    res.status(500).json({ error: 'Failed to update opportunity', message: err.message });
  }
});

// PATCH /api/pipeline/opportunities/:id/stage
// Change stage — logs to history, resets staleness timer
router.patch('/opportunities/:id/stage', requireRoles(...PIPELINE_READERS), async (req, res) => {
  try {
    ...
    if (stage === 'lost_paused' && !lost_reason) {
      return res.status(400).json({ error: 'lost_reason is required when marking a deal as Lost/Paused' });
    }
    ...
    if (req.user.role === 'brand_admin' && existing.lead_ba_id !== req.user.id) {
      return res.status(403).json({ error: 'You can only change stage on your own opportunities' });
    }
    ...
    res.json({ opportunity: updated, message: `Stage updated to "${stage}"` });
  } catch (err) {
    console.error('[Pipeline] changeStage error:', err);
    res.status(500).json({ error: 'Failed to change stage', message: err.message });
  }
});

// DELETE /api/pipeline/opportunities/:id
// Admin/MD/Super Admin only
router.delete('/opportunities/:id', requireRoles(...PIPELINE_ADMINS), async (req, res) => {
  try {
    const existing = await pipelineService.getOpportunityById(req.params.id);
    if (!existing) return res.status(404).json({ error: 'Opportunity not found' });
...
router.get('/opportunities/:id/notes', requireRoles(...PIPELINE_READERS), async (req, res) => {
  try {
    const notes = await pipelineService.getWeeklyNotes(req.params.id);
    res.json({ notes });
  } catch (err) {
    res.status(500).json({ error: 'Failed to load notes', message: err.message });
  }
});

// POST /api/pipeline/opportunities/:id/notes
// Upsert this week's note (creates or updates for current week)
router.post('/opportunities/:id/notes', requireRoles(...PIPELINE_READERS), async (req, res) => {
  try {
    ...
    if (req.user.role === 'brand_admin' && existing.lead_ba_id !== req.user.id) {
      return res.status(403).json({ error: 'You can only add notes to your own opportunities' });
    }
...
router.post('/opportunities/:id/notes/aria-draft', requireRoles(...PIPELINE_READERS), async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to generate ARIA draft', message: err.message });
  }
});
...
router.get('/momentum', requireRoles(...PIPELINE_ADMINS), async (req, res) => {
  try {
    ...
    const forecast_note = await ariaService.buildForecastNote(analytics.weighted_forecast);

    res.json({ commentary, forecast_note, analytics });
  } catch (err) {
    console.error('[Pipeline] momentum error:', err);
    res.status(500).json({ error: 'Failed to generate momentum commentary', message: err.message });
  }
});
