  ...
  async (req, res) => {
    try {
      ...
      if (!brand_id)
        return res
          .status(400)
          .json({ success: false, error: "brand_id is required." });
      if (!req.files || req.files.length === 0) {
        return res
          .status(400)
          .json({ success: false, error: "At least one file is required." });
      }
      ...
      if (!SUPER_ROLES.has(req.user.role)) {
        ...
        if (!ok)
          return res
            .status(403)
            .json({ success: false, error: "Not your brand." });
      }
      ...
      const { parsed: parsedDocs, errors: parseErrors } = await parseDocuments(
        req.files,
      );
      ...
      const docRecords = await Promise.all(
        req.files.map(async (file, i) => {
          ...
          let documentId = null;
          try {
            const { data, error } = await supabase.from("goal_source_documents")
              .insert({
                brand_id,
            ...
            if (error) {
              console.error(
                `[goal-generator] insert failed for ${file.originalname}:`,
                error.message,
              );
            } else {
            ...
            console.error(
              `[goal-generator] insert threw for ${file.originalname}:`,
              err.message,
            );
          }

          return { fileIndex: i, documentId };
        }),
      );
      ...
      if (primaryDocId && result.brief_intelligence) {
        try {
          const { error } = await supabase.from("goal_source_documents")
            .update({
          ...
          if (error)
            console.error(
              "[goal-generator] brief_intelligence update failed:",
              error.message,
            );
        } catch (err) {
          console.error(
            "[goal-generator] brief_intelligence update threw:",
            err.message,
          );
        }
      }
...
router.post("/save", authenticate, async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ success: true, goals: saved, created: saved.length });
  } catch (err) {
...
router.delete("/:goalId", authenticate, async (req, res) => {
  try {
    const data = await deleteGoalDirect({
      goalId: req.params.goalId,
      caller: req.user,
    });
...
// ── PATCH /api/goals/change-requests/:requestId/decide (SA approves/denies) ──
router.patch(
  "/change-requests/:requestId/decide",
  authenticate,
  async (req, res) => {
    try {
      ...
      if (typeof approve !== "boolean") {
        return res
          .status(400)
          .json({ success: false, error: "approve (true|false) is required." });
      }
