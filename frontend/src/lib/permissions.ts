...
export const BRAND_ROLE_META: Record<
  string,
  { label: string; icon: string; desc: string; isElevated: boolean }
> = {
  brand_admin: {
    label: "Brand Admin",
    icon: "🛡️",
    desc: "Full control over this brand only",
    isElevated: true,
  },
  account_manager: {
    label: "Account Manager",
    icon: "👔",
    desc: "Overall client relationship",
    isElevated: false,
  },
  brand_manager: {
    label: "Brand Manager",
    icon: "🎯",
    desc: "Day-to-day brand stewardship",
    isElevated: false,
  },
  creative_director: {
    label: "Creative Director",
    icon: "🎨",
    desc: "Oversees all creative output",
    isElevated: false,
  },
  senior_strategist: {
    label: "Senior Strategist",
    icon: "🧠",
    desc: "Leads strategy and planning",
    isElevated: false,
  },
  strategist: {
    label: "Strategist",
    icon: "📊",
    desc: "Executes campaign strategies",
    isElevated: false,
  },
  copywriter: {
    label: "Copywriter",
    icon: "✍️",
    desc: "Writes all brand copy",
    isElevated: false,
  },
  social_media_manager: {
    label: "Social Media Manager",
    icon: "📱",
    desc: "Social content and scheduling",
    isElevated: false,
  },
  analytics_specialist: {
    label: "Analytics Specialist",
    icon: "📈",
    desc: "Data and ClarityScore™",
    isElevated: false,
  },
  content_creator: {
    label: "Content Creator",
    icon: "🎬",
    desc: "Video and content assets",
    isElevated: false,
  },
  art_director: {
    label: "Art Director",
    icon: "🖌️",
    desc: "Visual assets and design",
    isElevated: false,
  },
  community_manager: {
...
export function canApproveDeliverables(
  systemRole?: string,
  roleOnBrand?: string,
): boolean {
  return canManageBrand(systemRole, roleOnBrand);
...
export function canPublishReports(
  systemRole?: string,
  roleOnBrand?: string,
): boolean {
  return canManageBrand(systemRole, roleOnBrand);
...
// ── getRoleOnBrand — plain async helper, not a hook ──────────────────────────
async function getRoleOnBrand(brandId: string): Promise<string | null> {
  try {
...
export function useBrandPermissions(brandId?: string) {
  ...
  const [roleOnBrand, setRoleOnBrand] = useState<string | null>(null);
  ...
  useEffect(() => {
    ...
    const cached = brandAssignments.find(
      (a: any) => a.brand_id === brandId || a.id === brandId,
    );

    if (cached?.role_on_brand) {
      setRoleOnBrand(cached.role_on_brand);
      setLoading(false);
    } else {
      // Fall back to API
      getRoleOnBrand(brandId).then((role) => {
        setRoleOnBrand(role);
        setLoading(false);
      });
    }
  ...
  // Global admins bypass brand-level checks entirely
