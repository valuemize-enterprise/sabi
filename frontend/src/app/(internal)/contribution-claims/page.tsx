                      ...
                      <div className="grid grid-cols-3 gap-2 mb-3">
                        {TIERS.map(t => (
                          <button key={t.value} onClick={() => setSelectedTier(p => ({ ...p, [c.id]: t.value }))}
                            className={`p-3 rounded-xl border text-center transition-all ${selectedTier[c.id] === t.value ? 'border-green-500/40 bg-green-500/10' : 'border-white/8 hover:border-white/15'
                              }`}>
