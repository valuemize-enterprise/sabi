...
export default function BrandIdentityPage() {
  ...
  const handleLogoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    setLogoFile(file);                         // actual file for upload
    setLogoUrl(URL.createObjectURL(file));     // blob URL just for preview
  };
  ...
  const save = async () => {
    ...
    try {
      ...
      // Append logo file if user picked a new one
      if (logoFile) {
      ...
      const fields: Record<string, any> = {
        ...
        brand_voice: JSON.stringify(voice),
        brand_colors: JSON.stringify(colors),
        dos_and_donts: JSON.stringify({ dos, donts }),
        brand_guidelines_url: guidelinesUrl || '',
        brand_assets: JSON.stringify(assets),
      };
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/agency/brands/${brandId}/identity`,
        {
          method: 'PATCH',
            ...
            <>
              ...
              <input ref={logoRef} type="file" accept="image/*" className="sr-only"
                onChange={e => {
                  ...
                  if (!f) return;
                  setLogoFile(f);                        // ← add this — stores the actual file
                  setLogoUrl(URL.createObjectURL(f));    // ← keep this — blob URL just for preview
                }}
              />
            </>
          )}
      ...
      <div className="flex items-center gap-1 p-1 bg-white/3 rounded-xl border border-white/5 mb-5 overflow-x-auto">
        {TABS.map(({ key, label, icon: Icon }) => (
          <button key={key} onClick={() => setActiveTab(key as any)}
            className={`flex items-center gap-1.5 px-4 py-2 text-sm rounded-lg transition-all whitespace-nowrap flex-shrink-0 ${activeTab === key ? 'bg-purple-600 text-white' : 'text-white/40 hover:text-white'
              }`}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {ARCHETYPES.map(a => (
              <button key={a.value} onClick={() => canEdit && setArchetype(a.value)}
                disabled={!canEdit}
                className={`flex items-start gap-3 p-3.5 rounded-2xl border text-left transition-all ${archetype === a.value
