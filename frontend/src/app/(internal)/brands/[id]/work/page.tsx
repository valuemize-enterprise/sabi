        ...
        {(['week','month','all'] as const).map(p => (
          <button key={p} onClick={() => setPeriod(p)}
            className={`px-3 py-1.5 text-xs rounded-lg capitalize transition-all ${
              period === p ? 'bg-purple-600 text-white' : 'text-white/40 hover:text-white'
            }`}>
            ...
            <form onSubmit={save} className="space-y-4">
              ...
              <div>
                ...
                <label className={`flex items-center gap-3 p-3 rounded-xl border border-dashed cursor-pointer transition-all ${
                  files.length ? 'border-purple-500/30 bg-purple-500/5' : 'border-white/10 hover:border-white/20'
                }`}>
